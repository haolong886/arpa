...
//******************************
//*(int *)outBufs->descs[0].buf = *(int *)inBufs->descs[0].buf+0x1111;
/*
 *(int *)outBufs->descs[0].buf = 0xbb;
 Memory_cacheWbInv(inBufs->descs[0].buf, 4);
 Memory_cacheWbInv(outBufs->descs[0].buf, 4);
return 0;
*/
   //***********************
   ...
   if (fxnID == NULL_INDEX){
       ...
       switch ((fxnID & vendorMsk)>>VENDOR_TAG_SHIFT){
		...
		//status = C6ACCEL_TI_arpalibFunctionCall((void *)pFnArray, fxnID,(XDM1_BufDesc *)inBufs, (XDM1_BufDesc *)outBufs);
		//if (status !=0)
                 //   return status;
                //else
                 //   break;
