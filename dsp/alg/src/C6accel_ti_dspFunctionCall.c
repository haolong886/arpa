...
int C6ACCEL_TI_dsplibFunctionCall(void *pFnArray,int fxnID,XDM1_BufDesc *inBufs, XDM1_BufDesc *outBufs)
{
 ...
 Int32 fxnidMsk= FXN_ID_MASK;
		 ...
		 case (ARPA_A_B_FILTER_FXN_ID) :{
			arpa_a_b_filter_Params *C6ACCEL_TI_arpa_a_b_filter__paramPtr;
        		C6ACCEL_TI_arpa_a_b_filter__paramPtr = pFnArray;
                    
			if(((C6ACCEL_TI_arpa_a_b_filter__paramPtr->a_InArrID1)>INBUF15)|
       				((C6ACCEL_TI_arpa_a_b_filter__paramPtr->b_OutArrID1)>OUTBUF15)|
       				((C6ACCEL_TI_arpa_a_b_filter__paramPtr->size_a) = 0) |
       				((C6ACCEL_TI_arpa_a_b_filter__paramPtr->size_b) = 0)){
         		return(IUNIVERSAL_EPARAMFAIL);
			}

    		else  
        	arpa_a_b_filter((int *)inBufs->descs[C6ACCEL_TI_arpa_a_b_filter__paramPtr->a_InArrID1].buf,
                          	(int *)outBufs->descs[C6ACCEL_TI_arpa_a_b_filter__paramPtr->b_OutArrID1].buf,                          
                          	C6ACCEL_TI_arpa_a_b_filter__paramPtr->size_a,
                          	C6ACCEL_TI_arpa_a_b_filter__paramPtr->size_b);
			Memory_cacheWbInv(inBufs->descs[0].buf, inBufs->descs[0].bufSize);
		 }
